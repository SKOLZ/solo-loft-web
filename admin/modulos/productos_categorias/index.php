...
$nombre_tabla = 'Productos_Categorias';

$primary_key = 'ID_Categoria';

$prefix_tpl = 'productos_categorias';
...
$catidad_facturas = $db->getAll("SELECT COUNT(ID_Categoria) as Cant_Cat FROM Productos WHERE ID_Categoria = ". $_GET['id']);
...
switch($do) {
		...
		if (!empty($_POST)) {			
			if ((int)($_POST['indice']) > 0) {
				$query = "UPDATE $nombre_tabla SET 								
						...
						Categoria = ". Misc::post('categoria') .",
						Online = ". Misc::post('online', 'int') ."
						WHERE $primary_key = ". Misc::post('indice', 'int'); 						
			}else{
				$query = "INSERT INTO $nombre_tabla (Categoria,Online) VALUES (
	...
	case 'borrar_categoria':
		Misc::control('listar');
		$borrar_producto = "DELETE FROM Productos WHERE ID_Categoria = " . $_GET['id_categoria'];	
		$db->execute($borrar_producto);
		$borra_cat = "DELETE FROM Productos_Categorias WHERE ID_Categoria = " . $_GET['id_categoria'];	
	...
	default:
		...
		$columnas_listado = array('Categoria');
		$campos_listado = array('Categoria');
		$campos_busqueda = array('Categoria');
